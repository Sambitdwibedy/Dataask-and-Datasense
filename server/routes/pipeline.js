...
const { loadSqliteToMysql, profileTable, hasSourceData, dropSourceData, syncColumnsFromSource } = require('../services/data-loader');
...
async function snapshotProgress(runId) {
  ...
  try {
    await query(
      `UPDATE pipeline_runs SET stages = ?, progress_snapshot = ? WHERE id = ?`,
...
router.post('/:appId/run', async (req, res) => {
  try {
    ...
    const [appCheckRows] = await query(
      'SELECT id, name, type, status, config FROM applications WHERE id = ?',
      [appId]
    );
    ...
    const [runningCheckRows] = await query(
      "SELECT id FROM pipeline_runs WHERE app_id = ? AND status = 'running'",
      [appId]
    );
    if (runningCheckRows.length > 0) {
    ...
    const [runInsertResult] = await query(
      `INSERT INTO pipeline_runs (app_id, triggered_by, status, started_at, stages)
       VALUES (?, ?, ?, NOW(), ?)`,
      [appId, req.user.id, 'running', JSON.stringify(stages)]
    );

    const [runRows] = await query('SELECT * FROM pipeline_runs WHERE id = ?', [runInsertResult.insertId]);
    const run = runRows[0];

    await query("UPDATE applications SET status = 'enriching', updated_at = NOW() WHERE id = ?", [appId]);
...
async function runEnrichmentPipeline(runId, appId, app) {
  ...
  try {
    ...
    if (sqlitePath && !sourceDataAvailable) {
      ...
      await loadSqliteToMysql(appId, sqlitePath, { dropExisting: true });
    ...
    const [tablesRows] = await query(
      'SELECT id, table_name, entity_name FROM app_tables WHERE app_id = ? ORDER BY table_name',
      [appId]
    );
    ...
    // Count draft columns
    const [colCountRows] = await query(
      `SELECT COUNT(*) as cnt FROM app_columns ac
       JOIN app_tables at ON ac.table_id = at.id
       WHERE at.app_id = ? AND ac.enrichment_status = 'draft'`,
      [appId]
    );
    progress.tables_total = tables.length;
    progress.columns_total = parseInt(colCountRows[0].cnt);
      ...
      for (const table of tables) {
        try {
          const colNames = (await query(
            'SELECT column_name FROM app_columns WHERE table_id = ?',
            [table.id]
          )).map(r => r.column_name);
          ...
          await query('UPDATE app_tables SET row_count = ? WHERE id = ?', [profile.row_count, table.id]);
    ...
    for (const table of tables) {
      const [colsRows] = await query(
        `SELECT id, column_name, data_type, is_pk, is_fk, fk_reference
         FROM app_columns
         WHERE table_id = ? AND enrichment_status = 'draft'
         ORDER BY column_name`,
        [table.id]
      );
      if (colsRows.length > 0) {
        tableColumnsMap[table.id] = { table, columns: colsRows };
      }
    }
    ...
    await query('DELETE FROM app_relationships WHERE app_id = ?', [appId]);

    // Build lookup: table_name → { table, pkColumns }
    const tableLookup = {};
    for (const table of tables) {
      const [pkCols] = await query(
        `SELECT column_name FROM app_columns WHERE table_id = ? AND is_pk = 1`,
        [table.id]
      );
      tableLookup[table.table_name.toLowerCase()] = {
        table,
        pkColumns: pkCols.map(r => r.column_name)
      };
    }
    ...
    for (const table of tables) {
      ...
      const [allCols] = await query(
        `SELECT id, column_name, is_fk, fk_reference FROM app_columns WHERE table_id = ?`,
        [table.id]
      );
      ...
      for (const col of allCols) {
        ...
        if (targetEntry && targetEntry.table.id !== table.id) {
          ...
          try {
            await query(
              `INSERT IGNORE INTO app_relationships (app_id, from_table_id, from_column, to_table_id, to_column, rel_type, cardinality)
               VALUES (?, ?, ?, ?, ?, ?, ?)`,
            ...
            if (!col.is_fk) {
              await query('UPDATE app_columns SET is_fk = 1 WHERE id = ?', [col.id]);
            }
          } catch (relErr) {
    ...
    for (const table of tables) {
      const [cols] = await query(
        `SELECT id, column_name, is_pk, is_fk FROM app_columns WHERE table_id = ?`,
        [table.id]
      );
      tableColCounts[table.id] = cols.length;
    ...
    for (const [colName, entries] of Object.entries(allColsByName)) {
      ...
      for (const entry of entries) {
        ...
        try {
          ...
          if (!entry.isFK) {
            await query('UPDATE app_columns SET is_fk = 1 WHERE id = ?', [entry.col.id]);
          }
    ...
    try {
      ...
      if (!sourceAvail3b) {
        ...
        const [existRels3b] = await query(
          `SELECT from_table_id, from_column, to_table_id, to_column FROM app_relationships WHERE app_id = ?`,
          [appId]
        );
        const existRelSet3b = new Set(
          existRels3b.map(r => `${r.from_table_id}:${r.from_column}:${r.to_table_id}:${r.to_column}`)
        );

        // Pre-fetch row counts from app_tables (tables query may not include row_count)
        const rowCounts3b = {};
        const [rcRows] = await query('SELECT id, row_count FROM app_tables WHERE app_id = ?', [appId]);
        ...
        for (const [colName, entries] of Object.entries(allColsByName)) {
          ...
          for (const entry of entriesWithData) {
            try {
              ...
              const dcnt = parseInt(distinctRows[0]?.dcnt || 0);
              const tcnt = parseInt(distinctRows[0]?.tcnt || 0);
          ...
          for (const entry of entriesWithData) {
            ...
            try {
              ...
              if (childSample.length === 0) continue;

              const sampleValues = childSample.map(r => r.val);
              ...
              const matchCount = parseInt(anchorMatch[0]?.cnt || 0);
              ...
              if (overlapRatio >= 0.5 && matchCount >= minMatches) {
                ...
                await query(
                  `INSERT IGNORE INTO app_relationships (app_id, from_table_id, from_column, to_table_id, to_column, rel_type, cardinality, confidence_score)
                   VALUES (?, ?, ?, ?, ?, 'inferred', 'many_to_one', ?)`,
    ...
    try {
      ...
      const sourceTableSet = new Set(sourceTablesS4.map(r => r.tbl));
      ...
      for (const pattern of FK_METADATA_PATTERNS) {
        ...
        try {
          ...
          console.log(`    ${pattern.table}: ${fkRows.length} FK entries, ${patternRelCount} matched schema, ${patternSkipped} skipped (not in schema)`);
      ...
      if (sourceTableSet.has('fnd_foreign_keys') && !sourceTableSet.has('fnd_resolved_fks')) {
        ...
        try {
          const fndFKResult = await executeOnSourceData(appId, `
            ...
            WHERE (fk."ENABLED_FLAG" IS NULL OR fk."ENABLED_FLAG" != 'N')
            ORDER BY ft_from."TABLE_NAME", fc_from."COLUMN_NAME"
          `, { maxRows: 0, timeout: 120000 });

          if (fndFKResult && fndFKRows.length > 0) {
            let fndRelCount = 0;
            let skippedNotInSchema = 0;

            for (const row of fndFKRows) {
              const fromTableName = row.from_table;
              const toTableName = row.to_table;
              const fromColumn = row.from_column;
              const toColumn = row.to_column || fromColumn;
            ...
            console.log(`    Oracle EBS raw JOINs: ${fndFKRows.length} declared FKs, ${fndRelCount} matched schema, ${skippedNotInSchema} skipped`);
    ...
    try {
      ...
      if (sourceAvailable) {
        ...
        const existingRelSet = new Set(
          existingRels.map(r => `${r.from_table_id}:${r.from_column}:${r.to_table_id}:${r.to_column}`)
        );
        ...
        for (const table of tables) {
          const [cols] = await query(
            `SELECT column_name, is_pk FROM app_columns WHERE table_id = ?`,
            [table.id]
          );
          ...
          for (const col of cols) {
            ...
            const alreadyMatched = existingRels.some(
        ...
        for (const pair of candidatePairs) {
          try {
            ...
            const matchCount = parseInt(parentMatch[0]?.cnt || 0);
            const overlapRatio = matchCount / sampleValues.length;
    ...
    const relCount = (await query('SELECT COUNT(*) as cnt FROM app_relationships WHERE app_id = ?', [appId]))[0].cnt;
    ...
    const [contextCheckRows] = await query(
      `SELECT COUNT(*) as count, COALESCE(SUM(LENGTH(extracted_text)), 0) as total_chars
       FROM context_documents WHERE app_id = ? AND extracted_text IS NOT NULL AND extracted_text != ''`,
      [appId]
    );
    const contextDocCount = parseInt(contextCheckRows[0].count);
    const contextChars = parseInt(contextCheckRows[0].total_chars);
    ...
    let skipEnrich = false;
    {
      ...
      const [draftColsRows] = await query(
        `SELECT COUNT(*) as col_count FROM app_columns
         WHERE table_id IN (SELECT id FROM app_tables WHERE app_id = ?)
         AND enrichment_status = 'draft'`,
        [appId]
      );
      const draftColCount = parseInt(draftColsRows[0].col_count);
    ...
    const [contextDocsRows] = await query(
    ...
    const userDocs = contextDocsRows.filter(d => d.file_type !== 'auto_discovered');
    ...
    const autoDiscoveredDoc = contextDocsRows.find(d => d.file_type === 'auto_discovered');
    ...
    const totalContextDocs = contextDocsRows.length;
    ...
    async function processEnrichResult(result, table) {
      ...
      if (result.token_usage) {
        ...
        try {
          ...
          await query(
            `INSERT IGNORE INTO token_usage (app_id, pipeline_run_id, stage, table_name, input_tokens, output_tokens, total_tokens, model, cost_estimate)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      ...
      for (const enrichedCol of result.columns) {
        ...
        await query(
          `UPDATE app_columns
           SET business_name = ?, description = ?, confidence_score = ?,
               enrichment_status = ?, enriched_by = 'ai', enriched_at = NOW(),
               value_mapping = ?, column_role = ?
           WHERE id = ?`,
          [
        ...
        if (enrichedCol.synonyms && enrichedCol.synonyms.length > 0) {
          for (const term of enrichedCol.synonyms) {
            try {
              await query(
                `INSERT IGNORE INTO app_synonyms (app_id, column_id, table_id, term, source, confidence_score, status, created_by)
                 VALUES (?, ?, ?, ?, 'ai_generated', 75, 'active', NULL) ON DUPLICATE KEY UPDATE status ='active', confidence_score = 75, updated_at = NOW()`,
      ...
      await query(
        `UPDATE app_tables SET description = ?, entity_metadata = ?, enrichment_status = 'ai_enriched', enriched_by = 'ai', enriched_at = NOW() WHERE id = ?`,
    ...
    try {
      const [enrichedTables] = await query(
        `SELECT id, table_name, entity_metadata FROM app_tables WHERE app_id = ? AND entity_metadata IS NOT NULL`,
        [appId]
      );
      ...
      for (const row of enrichedTables) {
      ...
      console.log(`Domain assignment: ${enrichedTables.length} tables → ${finalDomainCount} domains`);
    ...
    try {
      const [allCols] = await query(
        `SELECT ac.id, ac.column_name, ac.data_type, ac.is_pk, ac.is_fk, ac.column_role, ac.business_name,
         ...
         FROM app_columns ac
         JOIN app_tables at ON ac.table_id = at.id
         WHERE at.app_id = ? AND at.enrichment_status = 'ai_enriched'`,
        [appId]
      );
      ...
      for (const col of allCols) {
        ...
        if (newRole && newRole !== currentRole) {
          await query('UPDATE app_columns SET column_role = ? WHERE id = ?', [newRole, col.id]);
          if (currentRole) {
            heuristicOverrides++;
          } else {
            heuristicFills++;
      ...
      console.log(`Column role assignment: ${allCols.length} columns processed`);
      ...
      for (const col of colsForAlias) {
        ...
        if (match) {
          ...
          if (!desc.toLowerCase().includes('also known as') && !desc.toLowerCase().includes(match.aliases.split(',')[0].trim().toLowerCase())) {
            ...
            await query('UPDATE app_columns SET description = ? WHERE id = ?', [newDesc, col.id]);
            aliasesAdded++;
          }
        }
      }
    ...
    try {
      ...
      for (const tableRow of enrichedTablesForMeasures) {
        ...
        if (tableRow.table_name.match(/_HEADERS_ALL$/i)) {
          ...
          const linesTable = enrichedTablesForMeasures.find(t => t.table_name.toUpperCase() === linesTableName.toUpperCase());
          if (linesTable) {
        ...
        if (added) {
          ...
          await query('UPDATE app_tables SET entity_metadata = ? WHERE id = ?', [JSON.stringify(meta), tableRow.id]);
          measuresAdded += newMeasures.length - existingMeasures.length;
        }
      }

      console.log(`Computed measure discovery: ${measuresAdded} measures added across ${enrichedTablesForMeasures.length} tables`);
    } catch (measureErr) {
    ...
    await query('DELETE FROM query_patterns WHERE app_id = ?', [appId]);
    ...
    for (const t of enrichedTables) {
      ...
      for (const q of questions) {
        try {
          await query(
            `INSERT IGNORE INTO query_patterns (app_id, pattern_name, nl_template, sql_template, tables_used, status, usage_count, confidence)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    ...
    let skipValidate = false;
    {
      ...
      const [enrichedCount] = await query(
        `SELECT COUNT(*) as cnt FROM app_tables WHERE app_id = ? AND enrichment_status = 'ai_enriched'`,
        [appId]
      );
      const enrichedTableCount = parseInt(enrichedCount[0].cnt);
    ...
    try {
      ...
      if (sourceDataAvailableForQPD) {
        ...
        await query(
          `DELETE FROM test_queries WHERE app_id = ? AND confidence = 0.90`,
          [appId]
        );
        ...
        const [entitiesForQPD] = await query(
          `SELECT entity_name, table_name, entity_metadata FROM app_tables
           WHERE app_id = ? AND entity_metadata IS NOT NULL ORDER BY entity_name, table_name`,
          [appId]
        );

        const entityQuestions = {};
        for (const row of entitiesForQPD) {
          ...
          for (const question of entityData.questions) {
            try {
              ...
              if (!genResult.execError && genRows.length > 0) {
                ...
                seeded++;
                console.log(`  ✓ [${entityName}] "${question.substring(0, 50)}..." (${genRows.length} rows)`);
              } else {
                failed++;
    ...
    await query(
      `UPDATE pipeline_runs SET status = 'completed', completed_at = NOW(), stages = ? WHERE id = ?`,
      [JSON.stringify(finalStages), runId]
    );

    await query("UPDATE applications SET status = 'in_review', updated_at = NOW() WHERE id = ?", [appId]);
    ...
    if (isCancelled) {
      ...
      const finalStages = activeRuns[runId]?.stages || {};
      await query(
        `UPDATE pipeline_runs SET status = 'cancelled', completed_at = NOW(), stages = ? WHERE id = ?`,
        [JSON.stringify(finalStages), runId]
      );
      ...
      await query("UPDATE applications SET status = ?, updated_at = NOW() WHERE id = ?", [newAppStatus, appId]);
    } else {
      // --- REAL FAILURE ---
      ...
      await query("UPDATE applications SET status = 'profiling', updated_at = NOW() WHERE id = ?", [appId]);
    }
  }
}
...
async function updateStage(runId, stageName, status) {
  ...
  await query(
    `UPDATE pipeline_runs SET stages = ? WHERE id = ?`,
  ...
  try {
    ...
    const [rows] = await query(
      `SELECT id, app_id, status, started_at, completed_at, stages, progress_snapshot
       FROM pipeline_runs WHERE app_id = ? ORDER BY started_at DESC LIMIT 1`,
      [appId]
    );
  ...
  try {
    ...
    const [rows] = await query(
      `SELECT pr.id, pr.app_id, pr.triggered_by, pr.status, pr.started_at, pr.completed_at, pr.stages,
       ...
       WHERE pr.app_id = ? ORDER BY pr.started_at DESC LIMIT ?`,
      [appId, limit]
    );
...
router.post('/:appId/cancel', async (req, res) => {
  try {
    ...
    const [runRows] = await query(
      "SELECT id FROM pipeline_runs WHERE app_id = ? AND status = 'running' ORDER BY started_at DESC LIMIT 1",
      [appId]
    );
    ...
    if (progress) {
      ...
      await query(
        `UPDATE pipeline_runs SET status = 'cancelled', completed_at = NOW() WHERE id = ?`,
        [runId]
      );
      await query("UPDATE applications SET status = 'profiling', updated_at = NOW() WHERE id = ?", [appId]);
...
router.post('/:appId/reset', async (req, res) => {
  try {
    ...
    const [appCheckRows] = await query('SELECT id, name FROM applications WHERE id = ?', [appId]);
    if (appCheckRows.length === 0) {
      return res.status(404).json({ error: 'Application not found' });
    }
    ...
    await query(
      `UPDATE app_tables SET description = NULL, row_count = 0, entity_metadata = NULL WHERE app_id = ?`,
      [appId]
    );

    // Clear pipeline runs, curation logs, test queries, patterns, relationships, token usage
    await query('DELETE FROM pipeline_runs WHERE app_id = ?', [appId]);
    await query(
      `DELETE FROM curation_log WHERE column_id IN (
        SELECT ac.id FROM app_columns ac JOIN app_tables at ON ac.table_id = at.id WHERE at.app_id = ?
      )`,
      [appId]
    );
    await query('DELETE FROM test_queries WHERE app_id = ?', [appId]);
    await query('DELETE FROM query_patterns WHERE app_id = ?', [appId]);
    await query('DELETE FROM app_relationships WHERE app_id = ?', [appId]);
    await query('DELETE FROM token_usage WHERE app_id = ?', [appId]);
    ...
    await query(
      `UPDATE applications SET config = JSON_REMOVE(COALESCE(config, '{}'), '$.enrichment_snapshot') WHERE id = ?`,
      [appId]
    );

    // Reset app status
    await query("UPDATE applications SET status = 'draft', updated_at = NOW() WHERE id = ?", [appId]);

    res.json({
      message: `Application "${appCheckRows[0].name}" fully reset (including source data)`,
      columns_reset: colReset.affectedRows,
    });
  } catch (err) {
...
router.post('/:appId/reset-enrichments', async (req, res) => {
  try {
    ...
    const [appCheckRows2] = await query('SELECT id, name FROM applications WHERE id = ?', [appId]);
    if (appCheckRows2.length === 0) {
      return res.status(404).json({ error: 'Application not found' });
    }

    // Reset column enrichments but keep value_mapping (profiling data)
    const [colReset2] = await query(
      `UPDATE app_columns
       SET business_name = NULL, description = NULL, confidence_score = 0,
           enrichment_status = 'draft', enriched_by = NULL, enriched_at = NULL
       WHERE table_id IN (SELECT id FROM app_tables WHERE app_id = ?)`,
    ...
    await query(
      `UPDATE app_tables SET description = NULL, entity_metadata = NULL WHERE app_id = ?`,
      [appId]
    );
    ...
    await query("UPDATE applications SET status = 'profiled', updated_at = NOW() WHERE id = ?", [appId]);

    res.json({
      message: `Enrichments reset for "${appCheckRows2[0].name}" — source data and profiling preserved`,
      columns_reset: colReset2.affectedRows,
      data_preserved: true,
    });
  } catch (err) {
...
router.post('/:appId/re-enrich-with-context', async (req, res) => {
  ...
  try {
    step = 'app_check';
    const [appCheckRows3] = await query('SELECT id, name, type, status, config FROM applications WHERE id = ?', [appId]);
    if (appCheckRows3.length === 0) {
      return res.status(404).json({ error: 'Application not found' });
    }
    ...
    const snapshot = {
      ...
      column_count: snapshotRows.length,
      columns: snapshotRows.map(r => ({
    ...
    await query(
      `UPDATE applications
       SET config = JSON_SET(COALESCE(config, '{}'), '$.enrichment_snapshot', CAST(? AS JSON))
       WHERE id = ?`,
    ...
    step = 'reset_tables';
    await query(`UPDATE app_tables SET description = NULL, entity_metadata = NULL WHERE app_id = ?`, [appId]);
    ...
    const [runRows2] = await query('SELECT * FROM pipeline_runs WHERE id = ?', [runInsertRes.insertId]);

    step = 'post_insert';
    const run = runRows2[0];
    await query("UPDATE applications SET status = 'enriching', updated_at = NOW() WHERE id = ?", [appId]);
    ...
    step = 'send_response';
    res.json({
      message: `Re-enrichment started for "${appCheckRows3[0].name}" with context`,
      run: run,
...
router.get('/:appId/runs/:runId', async (req, res) => {
  try {
    ...
    const [rows] = await query(
      `SELECT * FROM pipeline_runs WHERE id = ? AND app_id = ?`,
      [runId, appId]
    );
...
router.post('/:appId/snapshot-enrichments', async (req, res) => {
  try {
    ...
    // Store snapshot in application config JSON
    await query(
      `UPDATE applications
       SET config = JSON_SET(COALESCE(config, '{}'), '$.enrichment_snapshot', CAST(? AS JSON))
       WHERE id = ?`,
...
router.get('/:appId/enrichment-diff', async (req, res) => {
  try {
    ...
    const [appRows] = await query(
      `SELECT config FROM applications WHERE id = ?`,
      [appId]
    );
    ...
    res.json({
      ...
      summary: {
        total_columns: currentRows.length,
...
router.get('/:appId/confidence-summary', async (req, res) => {
  try {
    ...
    const enrichedCount = rows.filter(r => r.enrichment_status && r.enrichment_status !== 'draft').length;
    if (enrichedCount === 0) {
      return res.json({ has_enrichments: false });
    }
    ...
    const [appRows] = await query(`SELECT config FROM applications WHERE id = ?`, [appId]);
    const hasSnapshot = !!(appRows[0]?.config?.enrichment_snapshot);
  ...
  try {
    ...
    const [existingDoc] = await query(
      `SELECT id, extracted_text FROM context_documents
       WHERE app_id = ? AND filename = 'user_knowledge_entries.csv'`,
      [appId]
    );
    ...
    if (existingDoc.length > 0) {
      ...
      await query(
        `UPDATE context_documents
         SET extracted_text = ?, file_size = ?, uploaded_at = NOW()
         WHERE id = ?`,
      ...
      const [insertResult] = await query(
        `INSERT INTO context_documents (app_id, filename, file_type, file_size, extracted_text, description, uploaded_by, uploaded_at)
         VALUES (?, 'user_knowledge_entries.csv', 'text/csv', ?, ?, 'Column knowledge entered directly by the builder operator via Context Coach', ?, NOW())`,
...
router.post('/:appId/enrich-only', async (req, res) => {
  const { appId } = req.params;
  try {
    const [appCheckRows4] = await query('SELECT id, name, type, config FROM applications WHERE id = ?', [appId]);
    if (appCheckRows4.length === 0) return res.status(404).json({ error: 'Application not found' });
    const app = appCheckRows4[0];

    // Get tables and columns
    const [tablesRows] = await query(
      'SELECT id, table_name, entity_name FROM app_tables WHERE app_id = ? ORDER BY table_name', [appId]);
    const tables = tablesRows;
    ...
    const [contextDocsRows] = await query(
      'SELECT filename, extracted_text FROM context_documents WHERE app_id = ? ORDER BY id', [appId]);
    const contextDocs = contextDocsRows;
    ...
    for (const table of tables) {
      ...
      try {
        const [columnsRows] = await query(
          'SELECT id, column_name, data_type, is_pk, is_fk, fk_reference, business_name, description FROM app_columns WHERE table_id = ?',
          [table.id]
        );
        ...
        for (const enrichedCol of allEnrichedCols) {
          ...
          await query(
            `UPDATE app_columns SET business_name = ?, description = ?, confidence_score = ?,
             enrichment_status = ?, enriched_by = 'ai', enriched_at = NOW(), value_mapping = COALESCE(?, value_mapping),
             column_role = ? WHERE id = ?`,
        ...
        await query(
          `UPDATE app_tables SET description = ?, entity_metadata = ?, enrichment_status = 'ai_enriched',
           enriched_by = 'ai', enriched_at = NOW() WHERE id = ?`,
    ...
    await query("UPDATE applications SET status = 'published', updated_at = NOW() WHERE id = ?", [appId]);
...
router.post('/:appId/run-step', async (req, res) => {
  try {
    ...
    const [appRows] = await query('SELECT id, name, type, status, config FROM applications WHERE id = ?', [appId]);
    ...
    if (action === 'complete' || action === 'skip') {
      ...
      await query(
        `UPDATE pipeline_steps SET status = ?, completed_at = NOW(), updated_at = NOW() WHERE app_id = ? AND step_name = ?`,
        [status, appId, step]
      );
    ...
    if (action === 'reset') {
      ...
      for (let i = 0; i < downstreamSteps.length; i++) {
        ...
        await query(
          `INSERT IGNORE INTO pipeline_steps (app_id, step_name, step_order, status, quality_score, quality_details, started_at, completed_at, updated_at)
           VALUES (?, ?, ?, 'not_started', NULL, NULL, NULL, NULL, NOW()) ON DUPLICATE KEY UPDATE status ='not_started', quality_score = NULL, quality_details = NULL,
      ...
      if (downstreamSteps.includes('synonyms')) {
        ...
        await query(
          `UPDATE app_synonyms SET status = 'inactive' WHERE app_id = ? AND source = 'ai_generated'`,
          [appId]
        );
      }
      // Mark KG as unpublished if any step before publish is reset
      if (stepIdx < STEP_ORDER.indexOf('publish')) {
        await query("UPDATE applications SET status = 'draft', updated_at = NOW() WHERE id = ? AND status = 'published'", [appId]);
      }
    ...
    if (deps.length > 0) {
      const depPlaceholders = deps.map(() => '?').join(', ');
      const [depCheck] = await query(
        `SELECT step_name, status FROM pipeline_steps WHERE app_id = ? AND step_name IN (${depPlaceholders})`,
        [appId, ...deps]
      );
      const incomplete = deps.filter(d => {
        const row = depCheck.find(r => r.step_name === d);
    ...
    if (app.status === 'published' && step !== 'publish') {
      const [currentStep] = await query('SELECT status FROM pipeline_steps WHERE app_id = ? AND step_name = ?', [appId, step]);
      if (currentStep[0]?.status === 'completed') {
        ...
        for (const ds of downstreamSteps) {
          await query(
            `UPDATE pipeline_steps SET status = 'not_started', quality_score = NULL, quality_details = NULL, started_at = NULL, completed_at = NULL, updated_at = NOW() WHERE app_id = ? AND step_name = ?`,
            [appId, ds]
          );
        }
      }
      ...
      switch (step) {
        ...
        case 'connect': {
          ...
          if (sqlitePath) {
            ...
            const [metaCheck] = await query('SELECT COUNT(*) as cnt FROM app_tables WHERE app_id = ?', [appId]);
            if (parseInt(metaCheck[0].cnt) > 0) {
              console.log(`[run-step connect] No SQLite file, but metadata exists (${metaCheck[0].cnt} tables). Reporting as connected.`);
            }
          }
          ...
          const [tableCount] = await query('SELECT COUNT(*) as cnt FROM app_tables WHERE app_id = ?', [appId]);
          const [colCount] = await query(
            `SELECT COUNT(*) as cnt FROM app_columns ac JOIN app_tables at ON ac.table_id = at.id WHERE at.app_id = ?`, [appId]
          );
          ...
          const emptyTables = tableList.filter(t => !t.row_count || parseInt(t.row_count) === 0);
          const t = parseInt(tableCount[0].cnt);
          const c = parseInt(colCount[0].cnt);
          const totalRows = tableList.reduce((sum, t) => sum + (parseInt(t.row_count) || 0), 0);
          result = {
            success: t > 0,
            freshLoad: loaded,
            sourceAvailable,
            ...
            columns: c,
            totalRows,
            tableList: tableList.slice(0, 20), // top 20 for display
          ...
          await query(
            `UPDATE pipeline_steps SET status = ?, quality_score = ?, quality_details = ?, completed_at = NOW(), updated_at = NOW() WHERE app_id = ? AND step_name = 'connect'`,
            [t > 0 ? 'completed' : 'needs_attention', t > 0 ? 100 : 0, JSON.stringify({ tables: t, columns: c, totalRows, emptyTables: emptyTables.length }), appId]
          );
          await query(
            `UPDATE pipeline_steps SET status = ?, quality_score = ?, quality_details = ?, completed_at = NOW(), updated_at = NOW() WHERE app_id = ? AND step_name = 'load'`,
            [t > 0 ? 'completed' : 'needs_attention', t > 0 ? 100 : 0, JSON.stringify({ tables: t, columns: c, totalRows, emptyTables: emptyTables.length }), appId]
          );
          break;
        }
        ...
        case 'load': {
          ...
          const [tables] = await query('SELECT COUNT(*) as cnt FROM app_tables WHERE app_id = ?', [appId]);
          const [cols] = await query(
            `SELECT COUNT(*) as cnt FROM app_columns ac JOIN app_tables at ON ac.table_id = at.id WHERE at.app_id = ?`, [appId]
          );
          const t = parseInt(tables[0].cnt);
          const c = parseInt(cols[0].cnt);
        ...
        case 'profile': {
          ...
          if (sourceAvail && sourceTables.length > 0) {
            for (const table of sourceTables) {
              try {
                const [colNamesRows] = await query('SELECT column_name FROM app_columns WHERE table_id = ?', [table.id]);
                const colNames = colNamesRows.map(r => r.column_name);
          ...
          if (profiledCount === 0 && sourceTables.length > 0) {
            const [existingProfilesRows] = await query(
              `SELECT COUNT(*) as cnt FROM app_tables WHERE app_id = ? AND row_count IS NOT NULL AND row_count > 0`, [appId]
            );
            existingProfileCount = parseInt(existingProfilesRows[0].cnt);
          ...
          const [profiledCols] = await query(
            `SELECT COUNT(*) as cnt FROM app_columns ac JOIN app_tables at ON ac.table_id = at.id
             WHERE at.app_id = ? AND (ac.data_type IS NOT NULL OR ac.value_mapping IS NOT NULL)`, [appId]
          );
          ...
          const [topTables] = await query(
            `SELECT table_name, row_count FROM app_tables WHERE app_id = ? AND row_count IS NOT NULL ORDER BY row_count DESC LIMIT 10`, [appId]
          );
          ...
          const [dateCols] = await query(
            `SELECT ac.column_name, at.table_name FROM app_columns ac JOIN app_tables at ON ac.table_id = at.id
             WHERE at.app_id = ? AND LOWER(ac.data_type) IN ('date', 'timestamp', 'datetime', 'timestamptz')`, [appId]
          );
          ...
          const [lowCardCols] = await query(
            `SELECT ac.column_name, at.table_name, ac.value_mapping FROM app_columns ac JOIN app_tables at ON ac.table_id = at.id
             WHERE at.app_id = ? AND ac.value_mapping IS NOT NULL AND ac.value_mapping != '{}'
          ...
          result = {
            ...
            profiledColumns: parseInt(profiledCols[0].cnt),
            totalColumns: parseInt(totalCols[0].cnt),
            topTables: topTables,
            dateColumns: dateCols,
            dateColumnCount: dateCols.length,
            lowCardColumns: lowCardCols.map(r => ({ column: r.column_name, table: r.table_name })),
            lowCardCount: lowCardCols.length,
            guidance: profiledCount > 0
              ? `Profiled ${profiledCount}/${total} tables. ${dateCols.length} date/temporal columns detected. ${lowCardCols.length} low-cardinality columns will get value dictionaries during enrichment. Review in Data Browser.`
              : existingProfileCount > 0
              ? `Using existing profile data for ${existingProfileCount}/${total} tables (source data not available for re-profiling). ${dateCols.length} date/temporal columns, ${lowCardCols.length} low-cardinality columns. Review in Data Browser.`
              : 'No tables profiled. Ensure source data is loaded (run Connect & Load first).',
          };
          await query(
            `UPDATE pipeline_steps SET status = ?, quality_score = ?, quality_details = ?, completed_at = NOW(), updated_at = NOW() WHERE app_id = ? AND step_name = 'profile'`,
            [stepSuccess ? 'completed' : 'needs_attention', score, JSON.stringify({ profiledTables: profiledCount, existingProfiles: existingProfileCount, totalTables: total, dateCols: dateCols.length, lowCardCols: lowCardCols.length }), appId]
          );
          break;
        }
        ...
        case 'discover': {
          ...
          const discTables = tablesRows;
          await query('DELETE FROM app_relationships WHERE app_id = ?', [appId]);

          // Build lookup
          const tblLookup = {};
          for (const t of discTables) {
            const [pkCols] = await query('SELECT column_name FROM app_columns WHERE table_id = ? AND is_pk = 1', [t.id]);
            tblLookup[t.table_name.toLowerCase()] = { table: t, pkColumns: pkCols.map(r => r.column_name) };
          }

          // Strategy 1+2: FK flag + naming convention
          for (const table of discTables) {
            const [allCols] = await query('SELECT id, column_name, is_fk, fk_reference FROM app_columns WHERE table_id = ?', [table.id]);
            for (const col of allCols) {
              ...
              if (targetEntry && targetEntry.table.id !== table.id) {
                ...
                await query(
                  `INSERT IGNORE INTO app_relationships (app_id, from_table_id, from_column, to_table_id, to_column, rel_type, cardinality)
                   VALUES (?, ?, ?, ?, ?, 'fk', 'many_to_one')`,
                  [appId, table.id, col.column_name, targetEntry.table.id, toColumn]
                );
                if (!col.is_fk) await query('UPDATE app_columns SET is_fk = 1 WHERE id = ?', [col.id]);
              }
            }
          }
          ...
          const tblColCounts = {};
          for (const t of discTables) {
            const [cols] = await query('SELECT id, column_name, is_pk, is_fk FROM app_columns WHERE table_id = ?', [t.id]);
            tblColCounts[t.id] = cols.length;
            for (const c of cols) {
          ...
          for (const [cn, entries] of Object.entries(colsByName)) {
            ...
            for (const entry of entries) {
              ...
              if (!entry.isFK) await query('UPDATE app_columns SET is_fk = 1 WHERE id = ?', [entry.col.id]);
            }
          }
          ...
          try {
            ...
            if (s3bSrcAvail) {
              ...
              const [existRels3bRows] = await query(
                `SELECT from_table_id, from_column, to_table_id, to_column FROM app_relationships WHERE app_id = ?`, [appId]
              );
              const existSet3b = new Set(existRels3bRows.map(r => `${r.from_table_id}:${r.from_column}:${r.to_table_id}:${r.to_column}`));

              // Pre-fetch row counts
              const rc3b = {};
              for (const t of discTables) {
                const [rcRow] = await query('SELECT row_count FROM app_tables WHERE id = ?', [t.id]);
                rc3b[t.id] = parseInt(rcRow[0]?.row_count || 0);
              }
                ...
                for (const entry of withData) {
                  try {
                    const dr = await execS3b(appId,
                      ...
                      { timeout: 5000 });
                    const d = parseInt(dr[0]?.d || 0), t = parseInt(dr[0]?.t || 0);
                    if (t === 0 || d === 0) continue;
                    const r = d / t;
          ...
          try {
            ...
            if (s4avail) {
              ...
              const srcSet = new Set(srcTbls.map(r => r.tbl));
              ...
              for (const pat of FK_PATTERNS) {
                ...
                try {
                  ...
                  for (const row of fkr) {
                    ...
                    await query(
                      `INSERT IGNORE INTO app_relationships (app_id, from_table_id, from_column, to_table_id, to_column, rel_type, cardinality, confidence_score)
                       VALUES (?, ?, ?, ?, ?, 'fk', 'many_to_one', 95)`,
                      [appId, fe.table.id, row.from_column, te.table.id, row.to_column || row.from_column]);
                    s4cnt++;
                  }
                  console.log(`    ${pat.table}: ${fkr.length} entries, ${s4cnt} matched`);
              ...
              if (srcSet.has('fnd_foreign_keys') && !srcSet.has('fnd_resolved_fks')) {
                try {
                  const fndRaw = await execS4(appId, `
                    ...
                    WHERE (fk."ENABLED_FLAG" IS NULL OR fk."ENABLED_FLAG" != 'N')`, { maxRows: 0, timeout: 120000 });
                  for (const row of fndRaw) {
                    if (!allTblNamesLower.includes(row.from_table?.toLowerCase()) ||
                        !allTblNamesLower.includes(row.to_table?.toLowerCase())) continue;
                    const fe = tblLookup[row.from_table.toLowerCase()];
                    const te = tblLookup[row.to_table.toLowerCase()];
          ...
          const [relCount] = await query('SELECT COUNT(*) as cnt FROM app_relationships WHERE app_id = ?', [appId]);
          const rels = parseInt(relCount[0].cnt);
          ...
          const [relDetails] = await query(
            `SELECT ft.table_name as from_table, ar.from_column, tt.table_name as to_table, ar.to_column, ar.cardinality
             ...
             WHERE ar.app_id = ? ORDER BY ft.table_name LIMIT 25`, [appId]
          );
          ...
          await query(
            `UPDATE pipeline_steps SET status = ?, quality_score = ?, quality_details = ?, completed_at = NOW(), updated_at = NOW() WHERE app_id = ? AND step_name = 'discover'`,
        ...
        case 'context': {
          ...
          const [docs] = await query(
            `SELECT COUNT(*) as cnt FROM doc_sources WHERE app_id = ?`, [appId]
          );
          const d = parseInt(docs[0].cnt);
          result = {
        ...
        case 'enrich': {
          ...
          const [enrichRunningCheck] = await query(
            "SELECT id FROM pipeline_runs WHERE app_id = ? AND status = 'running'", [appId]
          );
          if (enrichRunningCheck.length > 0) {
            delete activeStepRuns[appId];
          ...
          await query(
            `UPDATE pipeline_steps SET status = 'in_progress', started_at = NOW(), updated_at = NOW() WHERE app_id = ? AND step_name = 'enrich'`, [appId]
          );
          ...
          runEnrichmentPipeline(enrichRunId, appId, app).then(async () => {
            ...
            try {
              const [enrichedCount] = await query(
                `SELECT COUNT(*) as cnt FROM app_columns ac JOIN app_tables at ON ac.table_id = at.id
                 WHERE at.app_id = ? AND ac.enrichment_status IN ('ai_enriched', 'approved')`, [appId]
              );
              const [totalColCount] = await query(
                `SELECT COUNT(*) as cnt FROM app_columns ac JOIN app_tables at ON ac.table_id = at.id WHERE at.app_id = ?`, [appId]
              );
              const ec = parseInt(enrichedCount[0].cnt);
              const tcc = parseInt(totalColCount[0].cnt);
              const score = tcc > 0 ? Math.round((ec / tcc) * 100) : 0;
              await query(
                `UPDATE pipeline_steps SET status = 'completed', quality_score = ?, quality_details = ?, completed_at = NOW(), updated_at = NOW() WHERE app_id = ? AND step_name = 'enrich'`,
            ...
            try {
              await query(
                `UPDATE pipeline_steps SET status = 'needs_attention', quality_details = ?, updated_at = NOW() WHERE app_id = ? AND step_name = 'enrich'`,
        ...
        case 'synonyms': {
          const [syns] = await query(
            `SELECT COUNT(*) as cnt, COUNT(DISTINCT column_id) as cols_covered FROM app_synonyms WHERE app_id = ? AND status = 'active'`, [appId]
          );
          const [totalCols] = await query(
            `SELECT COUNT(*) as cnt FROM app_columns ac JOIN app_tables at ON ac.table_id = at.id WHERE at.app_id = ?`, [appId]
          );
          const s = parseInt(syns[0].cnt);
          const covered = parseInt(syns[0].cols_covered);
          const t = parseInt(totalCols[0].cnt);
          ...
          await query(
            `UPDATE pipeline_steps SET status = ?, quality_score = ?, quality_details = ?, completed_at = NOW(), updated_at = NOW() WHERE app_id = ? AND step_name = 'synonyms'`,
        ...
        case 'curate': {
          ...
          const [approvedRows] = await query(
            `SELECT COUNT(*) as cnt FROM app_columns ac JOIN app_tables at ON ac.table_id = at.id
             WHERE at.app_id = ? AND ac.enrichment_status = 'approved'`, [appId]
          );
          const [aiEnrichedRows] = await query(
            `SELECT COUNT(*) as cnt FROM app_columns ac JOIN app_tables at ON ac.table_id = at.id
             WHERE at.app_id = ? AND ac.enrichment_status = 'ai_enriched'`, [appId]
          );
          const t = parseInt(totalCols[0].cnt);
          const a = parseInt(approvedRows[0].cnt);
          const ai = parseInt(aiEnrichedRows[0].cnt);
          result = {
        ...
        case 'index': {
          await query(
            `UPDATE pipeline_steps SET status = 'in_progress', started_at = NOW(), updated_at = NOW() WHERE app_id = ? AND step_name = 'index'`, [appId]
          );
          ...
          if (embResult.generated > 0) {
            ...
            await query(
              'UPDATE applications SET config = ?, updated_at = NOW() WHERE id = ?',
          ...
          let synonymCount = 0;
          try {
            const [synRes] = await query(`SELECT COUNT(*) as cnt FROM app_synonyms WHERE app_id = ? AND status = 'active'`, [appId]);
            synonymCount = parseInt(synRes[0].cnt) || 0;
          } catch (e) { /* ignore */ }
          ...
          await query(
            `UPDATE pipeline_steps SET status = ?, quality_score = ?, quality_details = ?, ${success ? 'completed_at = NOW(),' : ''} updated_at = NOW() WHERE app_id = ? AND step_name = 'index'`,
        ...
        case 'validate': {
          ...
          try {
            const [testsRows] = await query(
              `SELECT COUNT(*) as total,
                      COUNT(CASE WHEN feedback = 'thumbs_up' THEN 1 END) as passed,
                      COUNT(CASE WHEN feedback = 'thumbs_down' THEN 1 END) as failed
               FROM test_queries WHERE app_id = ?`, [appId]
            );
            t = parseInt(testsRows[0].total);
            p = parseInt(testsRows[0].passed);
            f = parseInt(testsRows[0].failed);
            const [recentRows] = await query(
              `SELECT question, feedback, created_at FROM test_queries WHERE app_id = ? ORDER BY created_at DESC LIMIT 10`, [appId]
            );
          ...
          result = {
            ...
            recentTests,
            guidance: t > 0
              ? `?/${t} test queries passed (${score}%). ${f} failed. Review failing queries in Test Console and adjust enrichments or synonyms to improve accuracy.`
              : 'No test queries yet. Go to Test Console to ask sample questions and validate NL→SQL accuracy.',
            navigateTo: 'test',
          };
          try {
            await query(
              `UPDATE pipeline_steps SET status = ?, quality_score = ?, quality_details = ?, completed_at = NOW(), updated_at = NOW() WHERE app_id = ? AND step_name = 'validate'`,
          ...
          console.log(`[Workflow] Validate step completed for app ${appId}: ?/${t}`);
          break;
        }

        // ── STEP 9: PUBLISH ──
        case 'publish': {
          await query("UPDATE applications SET status = 'published', published_at = NOW(), published_by = ?, updated_at = NOW() WHERE id = ?", [req.user?.id || 1, appId]);

          // Auto-create workspace + assign all users (so app appears in Data Ask)
          let workspaceCreated = false;
          try {
            const [existingWs] = await query('SELECT id FROM workspaces WHERE app_id = ?', [appId]);
            if (existingWs.length === 0) {
              ...
              const wsId = wsInsert.insertId;
              // Assign all users to this workspace
              const [allUsers] = await query('SELECT id FROM users');
              for (const u of allUsers) {
                await query(
                  `INSERT IGNORE INTO workspace_members (workspace_id, user_id, role, is_default, enabled)
                   VALUES (?, ?, 'reader', TRUE, TRUE)`,
                  [wsId, u.id]
                );
              }
              workspaceCreated = true;
              console.log(`[Publish] Created workspace "${app.name}" (id=${wsId}) with ${allUsers.length} members`);
            }
          } catch (wsErr) {
          ...
          await query(
            `UPDATE pipeline_steps SET status = 'completed', quality_score = 100, completed_at = NOW(), updated_at = NOW() WHERE app_id = ? AND step_name = 'publish'`, [appId]
          );
          break;
        }
...
router.get('/:appId/step-status', async (req, res) => {
  try {
    ...
    const [profiledCols] = await query(
      `SELECT COUNT(*) as cnt FROM app_columns ac JOIN app_tables at ON ac.table_id = at.id
       WHERE at.app_id = ? AND ac.data_type IS NOT NULL`, [appId]
    );
    const [rels] = await query('SELECT COUNT(*) as cnt FROM app_relationships WHERE app_id = ?', [appId]);
    ...
    const [syns] = await query(
      `SELECT COUNT(*) as cnt FROM app_synonyms WHERE app_id = ? AND status = 'active'`, [appId]
    );
    const [tests] = await query(
      `SELECT COUNT(*) as total, COUNT(CASE WHEN feedback = 'thumbs_up' THEN 1 END) as passed FROM test_queries WHERE app_id = ?`, [appId]
    );
    const [appStatus] = await query('SELECT status, config FROM applications WHERE id = ?', [appId]);
    const cfg = appStatus[0]?.config || {};

    const t = parseInt(tables[0].cnt);
    const c = parseInt(cols[0].cnt);
    const pc = parseInt(profiledCols[0].cnt);
    const r = parseInt(rels[0].cnt);
    const e = parseInt(enriched[0].cnt);
    const a = parseInt(approved[0].cnt);
    const s = parseInt(syns[0].cnt);
    const tt = parseInt(tests[0].total);
    const tp = parseInt(tests[0].passed);

    // Check pipeline_steps for human step statuses and reset tracking
    let [pipelineSteps] = await query(
      `SELECT step_name, status FROM pipeline_steps WHERE app_id = ?`, [appId]
    );
    ...
    const stepMap = {};
    for (const ps of pipelineSteps) stepMap[ps.step_name] = ps.status;
    ...
    let docs = 0;
    try {
      const [docCount] = await query('SELECT COUNT(*) as cnt FROM doc_sources WHERE app_id = ?', [appId]);
      docs = parseInt(docCount[0].cnt);
    } catch {}
    ...
    try {
      const [domRes] = await query(
        `SELECT COUNT(DISTINCT COALESCE(NULLIF(NULLIF(NULLIF(JSON_UNQUOTE(JSON_EXTRACT(entity_metadata, '$.domain')), 'General'), 'UNKNOWN'), ''), 'Unclassified')) as cnt
         FROM app_tables WHERE app_id = ? AND entity_metadata IS NOT NULL`, [appId]);
      domainCount = parseInt(domRes[0].cnt) || 0;
    } catch {}
    ...
    res.json({
      ...
      publish:  { done: appStatus[0]?.status === 'published', summary: appStatus[0]?.status || 'draft' },
    });
  } catch (err) {
  ...
  try {
    ...
    const [tableSummary] = await query(
      `SELECT at.id, at.table_name, at.entity_name, at.row_count,
              COUNT(ac.id) as column_count,
              SUM(CASE WHEN ac.data_type IS NOT NULL THEN 1 ELSE 0 END) as profiled_columns,
              SUM(CASE WHEN LOWER(ac.data_type) IN ('date', 'timestamp', 'datetime', 'timestamptz') THEN 1 ELSE 0 END) as date_columns,
              SUM(CASE WHEN ac.value_mapping IS NOT NULL AND ac.value_mapping != '{}' THEN 1 ELSE 0 END) as low_card_columns
       FROM app_tables at
       LEFT JOIN app_columns ac ON at.id = ac.table_id
       WHERE at.app_id = ?
    ...
    // Summary stats
    const totalTables = tableSummary.length;
    const emptyTables = tableSummary.filter(t => !t.row_count || parseInt(t.row_count) === 0).length;
    const totalCols = columnDetail.length;
    const profiledCols = columnDetail.filter(c => c.data_type).length;
    const dateCols = columnDetail.filter(c => ['date', 'timestamp', 'datetime', 'timestamptz'].includes((c.data_type || '').toLowerCase()));
    const lowCardCols = columnDetail.filter(c => c.value_mapping && c.value_mapping !== '{}');
    const pkCols = columnDetail.filter(c => c.is_pk);
    const fkCols = columnDetail.filter(c => c.is_fk);

    // Data type distribution
    const typeDistribution = {};
    for (const c of columnDetail) {
...
router.put('/:appId/data-source', async (req, res) => {
  try {
    ...
    const [appRows] = await query('SELECT id, name, config FROM applications WHERE id = ?', [appId]);
    if (appRows.length === 0) return res.status(404).json({ error: 'Application not found' });
...
router.get('/:appId/data-source', async (req, res) => {
  try {
    ...
    const [appRows] = await query('SELECT id, name, type, config FROM applications WHERE id = ?', [appId]);
    if (appRows.length === 0) return res.status(404).json({ error: 'Application not found' });
    ...
    res.json({
      ...
      tablesInAKG: parseInt(tableCount[0].cnt),
      status: resolvedPath ? 'ok' : (sourceAvailable ? 'loaded_no_file' : (parseInt(tableCount[0].cnt) > 0 ? 'metadata_only' : 'not_configured')),
    });
  } catch (err) {
...
router.get('/:appId/diagnose-relationships', async (req, res) => {
  ...
  try {
    ...
    // Get all tables
    const [tablesRows] = await query('SELECT id, table_name, row_count FROM app_tables WHERE app_id = ?', [appId]);
    const tables = tablesRows;
    const totalTableCount = tables.length;
    const ubiquityThreshold = Math.max(5, Math.floor(totalTableCount * 0.5));
    ...
    res.json({
      ...
      existingRelationships: relSummary.length,
      relationshipList: relSummary,
